  ...
  methods: {
    ...
    async getPermissions() {
      ...
      this.authCodeList.forEach((authCode) => {
        ...
        authCode.children = this.generatePermissionTree(permissions, 0);
      });
    },

    generatePermissionTree(permissions, pid) {
      const result = [];
      for (let index = 0; index < permissions.length; ) {
        const permission = permissions[index];
        if (permission.pid === pid) {
          permissions.splice(index, 1);
          const chd = this.generatePermissionTree(permissions, permission.id);
          const pmn = {
            id: permission.id,
